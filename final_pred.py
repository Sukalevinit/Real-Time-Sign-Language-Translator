...
class Application:
    ...
    def __init__(self):
        ...
        # Indian languages dictionary with language codes
        ...
        self.space.place(x=1005, y=630)
        self.space.config(text="Space", font=("Courier", 20), wraplength=100, command=self.space_fun)

        # Add backspace button
        self.backspace = tk.Button(self.root)
        self.backspace.place(x=905, y=630)
        self.backspace.config(text="⌫", font=("Courier", 20), wraplength=100, command=self.backspace_fun)

        # Add language selection dropdown
        self.lang_var = tk.StringVar()
        self.lang_dropdown = ttk.Combobox(self.root, textvariable=self.lang_var, values=list(self.indian_languages.keys()))
        self.lang_dropdown.place(x=1105, y=580)
        self.lang_dropdown.set('Hindi')  # Default language

        # Add translate button
        self.translate = tk.Button(self.root)
        self.translate.place(x=1105, y=630)
        self.translate.config(text="Translate", font=("Courier", 20), wraplength=100, command=self.translate_fun)

        # Add translated text label
        self.translated_label = tk.Label(self.root)
        self.translated_label.place(x=260, y=680)
        self.translated_label.config(text="", font=("Courier", 15))
    ...
    def translate_fun(self):
        if self.str.strip():  # Check if there's text to translate
            try:
                selected_lang = self.indian_languages[self.lang_var.get()]
                translated = self.translator.translate(self.str.strip(), dest=selected_lang)
                self.translated_label.config(text=f"Translated: {translated.text}")
            except Exception as e:
                print(f"Translation error: {str(e)}")
                self.translated_label.config(text="Translation failed. Please try again.")
